# Flight Operations — Developer Implementation Guide
...
Flight Operations owns craft lifecycle aboard the host, bay assignment, traffic sequence, launch/recovery clearance, and servicing orchestration. Each craft retains separate vessel authority.
...
- IAttachedCraftService
- IHangarService
- ILaunchService
- IRecoveryService
- ISortieService
- ICraftServicingService
- IFlightTrafficService
...
| --- | --- | --- | --- |
| Flight Operations Dashboard | Flight_Operations_DashboardProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Craft Roster | Craft_RosterProjection | RegisterAttachedCraft, ReleaseCraft, ScrambleRescueCraft | Event-driven alarms; configured normal/fast telemetry |
| Hangar Map | Hangar_MapProjection | ReserveHangarBay | Event-driven alarms; configured normal/fast telemetry |
| Launch Control | Launch_ControlProjection | StartLaunchSequence, ClearLaunch | Event-driven alarms; configured normal/fast telemetry |
| Recovery Control | Recovery_ControlProjection | AssignRecoveryBay | Event-driven alarms; configured normal/fast telemetry |
| Approach Traffic | Approach_TrafficProjection | GrantApproach | Event-driven alarms; configured normal/fast telemetry |
| Bay Door and Pressure | Bay_Door_and_PressureProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Refuelling | RefuellingProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Rearming | RearmingProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Repair and Servicing | Repair_and_ServicingProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Pilot Assignments | Pilot_AssignmentsProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Sortie Planning | Sortie_PlanningProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Drone Launch Operations | Drone_Launch_OperationsProjection | StartLaunchSequence, ClearLaunch | Event-driven alarms; configured normal/fast telemetry |
| Search and Rescue | Search_and_RescueProjection | ScrambleRescueCraft | Event-driven alarms; configured normal/fast telemetry |
| Flight-Deck Safety | Flight_Deck_SafetyProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Tug and Retrieval | Tug_and_RetrievalProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
...
- Launch requires all configured gates or explicit waiver
- Hangar interlocks are server-side
- Docked craft state remains separate
- Servicing reconciles inventory
- Emergency recovery avoids duplicate bay assignment
