# Life Support and Habitation — Developer Implementation Guide
...
Life Support owns environmental zones, pressure operations, oxygen reserve policy, airlock cycles, gravity/lighting profiles, and survival estimates. Damage Control contains casualties and Engineering owns plant safety.
...
- IAirlockService
- IGravityZoneService
- ILightingZoneService
- IShelterService
- ISurvivalEstimateService
...
- DepressuriseZone
- StartOxygenPlant
- ReleaseOxygenReserve
- CycleAirlock
- AbortAirlockCycle
- SetGravityProfile
- SetLightingProfile
- OpenEmergencyShelter
...
| --- | --- | --- | --- |
| Life Support Dashboard | Life_Support_DashboardProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Atmosphere Overview | Atmosphere_OverviewProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Compartment Pressure | Compartment_PressureProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Oxygen Generation | Oxygen_GenerationProjection | StartOxygenPlant, ReleaseOxygenReserve | Event-driven alarms; configured normal/fast telemetry |
| Oxygen Storage | Oxygen_StorageProjection | StartOxygenPlant, ReleaseOxygenReserve | Event-driven alarms; configured normal/fast telemetry |
| Ventilation | VentilationProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Airlock Management | Airlock_ManagementProjection | CycleAirlock, AbortAirlockCycle | Event-driven alarms; configured normal/fast telemetry |
| Gravity Zones | Gravity_ZonesProjection | SetGravityProfile | Event-driven alarms; configured normal/fast telemetry |
| Lighting Zones | Lighting_ZonesProjection | SetLightingProfile | Event-driven alarms; configured normal/fast telemetry |
| Emergency Shelters | Emergency_SheltersProjection | OpenEmergencyShelter | Event-driven alarms; configured normal/fast telemetry |
| Crew Quarters and Occupancy | Crew_Quarters_and_OccupancyProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Water and Food | Water_and_FoodProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Waste Processing | Waste_ProcessingProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Temperature Model | Temperature_ModelProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Environmental Trends | Environmental_TrendsProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Survival Endurance | Survival_EnduranceProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
...
- Airlocks recover safely after restart
- Pressure actions check occupancy/boundaries
- Endurance exposes assumptions
- Emergency reserve is protected
- Damage and Life Support agree on pressure
