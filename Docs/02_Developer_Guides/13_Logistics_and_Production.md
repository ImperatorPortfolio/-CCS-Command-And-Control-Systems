# Logistics and Production — Developer Implementation Guide
...
Logistics owns stock accounting, reservations, transfer transactions, production intent, targets, and supply requests. Engineering owns physical conveyor and plant safety.
...
| --- | --- | --- | --- |
| Logistics Dashboard | Logistics_DashboardProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Cargo Manifest | Cargo_ManifestProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Storage Locations | Storage_LocationsProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Inventory Search | Inventory_SearchProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Ammunition Logistics | Ammunition_LogisticsProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Fuel Logistics | Fuel_LogisticsProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Component Reserves | Component_ReservesProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Ore and Ingot Reserves | Ore_and_Ingot_ReservesProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Refinery Control | Refinery_ControlProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Assembler Control | Assembler_ControlProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Production Planning | Production_PlanningProjection | CreateProductionJob, ApproveProductionPlan | Event-driven alarms; configured normal/fast telemetry |
| Conveyor Distribution | Conveyor_DistributionProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Automatic Stock Targets | Automatic_Stock_TargetsProjection | ReserveStock, SetStockTarget | Event-driven alarms; configured normal/fast telemetry |
| Loading and Unloading | Loading_and_UnloadingProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Supply Requests | Supply_RequestsProjection | CreateSupplyRequest | Event-driven alarms; configured normal/fast telemetry |
| Salvage Intake | Salvage_IntakeProjection | AcceptSalvage | Event-driven alarms; configured normal/fast telemetry |
| Waste and Scrap Processing | Waste_and_Scrap_ProcessingProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Logistics History | Logistics_HistoryProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
...
- Gross/access/reserved/ready stock are separate
- Interrupted transfers do not duplicate
- Automation is bounded
- Critical stock reservation remains visible
- Discrepancies create reconciliation record
