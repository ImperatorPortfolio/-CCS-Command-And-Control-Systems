# Operations — Developer Implementation Guide
...
Operations owns coordination state: tasks, schedules, requests, dependencies, readiness reports, and procedure orchestration. It does not own underlying departmental equipment or strategic campaign truth.
...
| --- | --- | --- | --- |
| Operations Dashboard | Operations_DashboardProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Operational Picture | Operational_PictureProjection | CreateOperationalTask | Event-driven alarms; configured normal/fast telemetry |
| Department Status | Department_StatusProjection | RequestDepartmentStatus | Event-driven alarms; configured normal/fast telemetry |
| Resource Allocation | Resource_AllocationProjection | ApplyResourcePlan | Event-driven alarms; configured normal/fast telemetry |
| Task Board | Task_BoardProjection | CreateOperationalTask, AssignTask | Event-driven alarms; configured normal/fast telemetry |
| Procedure Control | Procedure_ControlProjection | StartProcedure, PauseProcedure, AbortProcedure | Event-driven alarms; configured normal/fast telemetry |
| Dependency Map | Dependency_MapProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Watch Schedule | Watch_ScheduleProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Maintenance Coordination | Maintenance_CoordinationProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Mission Timeline | Mission_TimelineProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Request Queue | Request_QueueProjection | RequestDepartmentStatus | Event-driven alarms; configured normal/fast telemetry |
| Automation Queue | Automation_QueueProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Readiness Reports | Readiness_ReportsProjection | SubmitReadinessReport | Event-driven alarms; configured normal/fast telemetry |
...
- Operations can identify the exact blocker preventing readiness
- Tasks and procedures survive restart without duplicate execution
- Allocations cannot bypass department safety limits
- Completed tasks contain evidence or authorised waiver
- Automation is visible and cancellable
