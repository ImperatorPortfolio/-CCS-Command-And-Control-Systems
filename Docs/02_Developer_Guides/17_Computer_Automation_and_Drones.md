# Computer, Automation and Drones — Developer Implementation Guide
...
Computer owns platform health, automation definitions and journals, drone mission control, storage, failover, and digital security. Owning departments retain authority for commands automation invokes.
...
- INetworkTopologyService
- IRulesEngine
- IProcedureRepository
- IScheduler
- IDroneMissionService
...
| --- | --- | --- | --- |
| Computer Dashboard | Computer_DashboardProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Computer Core | Computer_CoreProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Network Map | Network_MapProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Automation Rules | Automation_RulesProjection | EnableAutomationRule, DisableAutomationRule, SimulateAutomationRule | Event-driven alarms; configured normal/fast telemetry |
| Sequence Library | Sequence_LibraryProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Task Scheduler | Task_SchedulerProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Drone Control | Drone_ControlProjection | AssignDroneMission, RecallDrone | Event-driven alarms; configured normal/fast telemetry |
| Autopilot Behaviours | Autopilot_BehavioursProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Remote Control | Remote_ControlProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Module Status | Module_StatusProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Data Storage | Data_StorageProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Failover Control | Failover_ControlProjection | InitiateFailover | Event-driven alarms; configured normal/fast telemetry |
| Performance | PerformanceProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Cybersecurity | CybersecurityProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Audit Logs | Audit_LogsProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
...
- Automation uses command contracts
- Scheduled work is idempotent
- Failover guarantees one authority
- Drone missions have bounds/abort/safe state
- Audit and backup failures are actionable
