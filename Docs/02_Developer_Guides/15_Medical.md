# Medical — Developer Implementation Guide
...
Medical owns gameplay casualty cases, triage, treatment queues, medical readiness, and quarantine request state. It does not diagnose real conditions or supersede Security and Life Support control.
...
- IMedicalStatusService
- ICasualtyService
- ITriageService
- ITreatmentCapacityService
- IExposureService
- IQuarantineService
- IMedicalEvacuationService
...
- CreateCasualtyCase
- SetTriagePriority
- AssignTreatmentFacility
- RequestMedicalRescue
- ReserveMedicalSupply
- InitiateQuarantine
- ReleaseQuarantine
...
| --- | --- | --- | --- |
| Medical Dashboard | Medical_DashboardProjection | RequestMedicalRescue, ReserveMedicalSupply, CloseMedicalIncident | Event-driven alarms; configured normal/fast telemetry |
| Crew Medical Status | Crew_Medical_StatusProjection | RequestMedicalRescue, ReserveMedicalSupply, CloseMedicalIncident | Event-driven alarms; configured normal/fast telemetry |
| Triage Board | Triage_BoardProjection | SetTriagePriority | Event-driven alarms; configured normal/fast telemetry |
| Casualty Location Map | Casualty_Location_MapProjection | CreateCasualtyCase | Event-driven alarms; configured normal/fast telemetry |
| Medical Bay Status | Medical_Bay_StatusProjection | RequestMedicalRescue, ReserveMedicalSupply, CloseMedicalIncident | Event-driven alarms; configured normal/fast telemetry |
| Treatment Queue | Treatment_QueueProjection | AssignTreatmentFacility | Event-driven alarms; configured normal/fast telemetry |
| Medical Supplies | Medical_SuppliesProjection | RequestMedicalRescue, ReserveMedicalSupply, CloseMedicalIncident | Event-driven alarms; configured normal/fast telemetry |
| Environmental Exposure | Environmental_ExposureProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Quarantine Control | Quarantine_ControlProjection | InitiateQuarantine, ReleaseQuarantine | Event-driven alarms; configured normal/fast telemetry |
| Medical Evacuation | Medical_EvacuationProjection | RequestMedicalRescue, ReserveMedicalSupply, CloseMedicalIncident | Event-driven alarms; configured normal/fast telemetry |
| Rescue Coordination | Rescue_CoordinationProjection | RequestMedicalRescue | Event-driven alarms; configured normal/fast telemetry |
| Medical Incident Log | Medical_Incident_LogProjection | RequestMedicalRescue, ReserveMedicalSupply, CloseMedicalIncident | Event-driven alarms; configured normal/fast telemetry |
...
- Only supported gameplay states are claimed
- Locations include time/confidence
- Capacity reflects actual blocks
- Quarantine cannot override life safety
- Records use minimum necessary detail
