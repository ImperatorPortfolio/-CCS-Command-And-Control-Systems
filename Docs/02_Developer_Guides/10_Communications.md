# Communications — Developer Implementation Guide
...
Communications owns links, delivery, channels, authentication, relay routing, and datalink transport. It does not own the truth of shared tracks, routes, or campaign orders.
...
- IChannelService
- IMessageQueue
- IRelayRoutingService
- IAuthenticationService
- IDatalinkService
- IPublicAddressService
...
| --- | --- | --- | --- |
| Communications Dashboard | Communications_DashboardProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Channel Control | Channel_ControlProjection | OpenChannel, CloseChannel | Event-driven alarms; configured normal/fast telemetry |
| Direct Communications | Direct_CommunicationsProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Fleet Communications | Fleet_CommunicationsProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Command Broadcast | Command_BroadcastProjection | BroadcastAnnouncement | Event-driven alarms; configured normal/fast telemetry |
| Distress and Emergency | Distress_and_EmergencyProjection | TransmitDistress | Event-driven alarms; configured normal/fast telemetry |
| Antenna Network | Antenna_NetworkProjection | SetAntennaProfile | Event-driven alarms; configured normal/fast telemetry |
| Relay Routing | Relay_RoutingProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Encryption and Authentication | Encryption_and_AuthenticationProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Tactical Datalink | Tactical_DatalinkProjection | ShareDatalinkItem | Event-driven alarms; configured normal/fast telemetry |
| Identification Challenge | Identification_ChallengeProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Message Queue | Message_QueueProjection | SendMessage, AcknowledgeMessage | Event-driven alarms; configured normal/fast telemetry |
| Recorded Transmissions | Recorded_TransmissionsProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Public Address | Public_AddressProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Automated Announcements | Automated_AnnouncementsProjection | BroadcastAnnouncement | Event-driven alarms; configured normal/fast telemetry |
| Remote Command Sessions | Remote_Command_SessionsProjection | GrantRemoteSession, TerminateRemoteSession | Event-driven alarms; configured normal/fast telemetry |
...
- Remote commands use normal validation
- Shared contacts retain source/time/uncertainty
- Emergency traffic pre-empts routine
- Emission state matches hardware
