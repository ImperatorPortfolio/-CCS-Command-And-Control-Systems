# Security — Developer Implementation Guide
...
Security owns access policy, lockdown, incidents, armory accountability, brig control, and cybersecurity response. Life-safety systems retain emergency override.
...
- ISecurityZoneService
- IPersonnelTrackingService
- IIncidentService
- ILockdownService
- IArmoryService
- ICybersecurityService
...
| --- | --- | --- | --- |
| Security Dashboard | Security_DashboardProjection | DispatchSecurityTeam | Event-driven alarms; configured normal/fast telemetry |
| Access Control | Access_ControlProjection | GrantTemporaryAccess, RevokeAccess | Event-driven alarms; configured normal/fast telemetry |
| Deck Security Map | Deck_Security_MapProjection | DispatchSecurityTeam | Event-driven alarms; configured normal/fast telemetry |
| Personnel Tracking | Personnel_TrackingProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Intrusion Detection | Intrusion_DetectionProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Lockdown Control | Lockdown_ControlProjection | InitiateLockdown, ReleaseLockdown | Event-driven alarms; configured normal/fast telemetry |
| Security Cameras | Security_CamerasProjection | DispatchSecurityTeam | Event-driven alarms; configured normal/fast telemetry |
| Armory | ArmoryProjection | SealArmory | Event-driven alarms; configured normal/fast telemetry |
| Brig | BrigProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Security Teams | Security_TeamsProjection | DispatchSecurityTeam | Event-driven alarms; configured normal/fast telemetry |
| Boarding Defence | Boarding_DefenceProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Sabotage Detection | Sabotage_DetectionProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
...
| Visitor Control | Visitor_ControlProjection | IssueVisitorPass | Event-driven alarms; configured normal/fast telemetry |
| Security Log | Security_LogProjection | DispatchSecurityTeam | Event-driven alarms; configured normal/fast telemetry |
...
- Lockdown reports trapped personnel and life-safety conflict
- Audit cannot be erased by operator
- Replay is rejected
- Internal defence is strictly bounded
