# Damage Control — Developer Implementation Guide
...
Damage Control owns casualty aggregation, compartment containment, repair priority, teams, and recovery orchestration. Owning departments retain equipment release authority.
...
- IDamageModelService
- ICompartmentService
- IAtmosphereCasualtyService
- IIsolationService
- IRepairQueueService
- IEmergencyRoutingService
- IRecoveryService
...
| --- | --- | --- | --- |
| Damage Control Dashboard | Damage_Control_DashboardProjection | AssignDamageTeam | Event-driven alarms; configured normal/fast telemetry |
| Damage Map | Damage_MapProjection | AssignDamageTeam | Event-driven alarms; configured normal/fast telemetry |
| Hull Integrity | Hull_IntegrityProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Atmosphere Loss | Atmosphere_LossProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Compartment Isolation | Compartment_IsolationProjection | SealCompartment, IsolateCompartmentPower | Event-driven alarms; configured normal/fast telemetry |
| Fire Control | Fire_ControlProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Structural Stability | Structural_StabilityProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| System Casualties | System_CasualtiesProjection | ReleaseRecoveredSystem | Event-driven alarms; configured normal/fast telemetry |
| Damage Teams | Damage_TeamsProjection | AssignDamageTeam | Event-driven alarms; configured normal/fast telemetry |
| Repair Queue | Repair_QueueProjection | CreateRepairTask | Event-driven alarms; configured normal/fast telemetry |
| Emergency Routing | Emergency_RoutingProjection | ActivateEmergencyRoute | Event-driven alarms; configured normal/fast telemetry |
| Evacuation Routes | Evacuation_RoutesProjection | PublishEvacuationRoute | Event-driven alarms; configured normal/fast telemetry |
| Casualty Estimates | Casualty_EstimatesProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Battle Damage Assessment | Battle_Damage_AssessmentProjection | AssignDamageTeam | Event-driven alarms; configured normal/fast telemetry |
| Recovery Procedures | Recovery_ProceduresProjection | StartRecoveryProcedure | Event-driven alarms; configured normal/fast telemetry |
...
- Damage map reflects actual state and configured topology
- Isolation previews personnel/system effects
- Repair does not clear fault without test
- Recovery survives restart
- Readiness reflects unresolved damage
