# Fleet and Strategic Operations — Developer Implementation Guide
...
Fleet owns task-group organisation, fleet orders, shared plans, and coordination state. Scenarium owns campaign, faction, conquest, and objective truth. Each vessel owns execution.
...
- ITaskGroupService
- IFleetOrderService
- IFormationPlanService
- IFleetDataLinkService
- IJumpWindowService
- IScenariumGateway
...
- AssignFleetUnit
- IssueFleetOrder
- AcknowledgeFleetOrder
- PublishSharedTrack
- PublishSharedRoute
- CreateJumpWindow
- ReportJumpReady
- RequestReinforcement
- SubmitCampaignEvidence
...
| --- | --- | --- | --- |
| Fleet Dashboard | Fleet_DashboardProjection | AssignFleetUnit, IssueFleetOrder, AcknowledgeFleetOrder | Event-driven alarms; configured normal/fast telemetry |
| Fleet Tactical Picture | Fleet_Tactical_PictureProjection | AssignFleetUnit, IssueFleetOrder, AcknowledgeFleetOrder | Event-driven alarms; configured normal/fast telemetry |
| Task-Group Organisation | Task_Group_OrganisationProjection | CreateTaskGroup | Event-driven alarms; configured normal/fast telemetry |
| Formation Control | Formation_ControlProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Fleet Orders | Fleet_OrdersProjection | AssignFleetUnit, IssueFleetOrder, AcknowledgeFleetOrder | Event-driven alarms; configured normal/fast telemetry |
| Shared Contacts | Shared_ContactsProjection | PublishSharedTrack, PublishSharedRoute | Event-driven alarms; configured normal/fast telemetry |
| Shared Navigation | Shared_NavigationProjection | PublishSharedTrack, PublishSharedRoute | Event-driven alarms; configured normal/fast telemetry |
| Jump Synchronisation | Jump_SynchronisationProjection | CreateJumpWindow, ReportJumpReady | Event-driven alarms; configured normal/fast telemetry |
| Asset Readiness | Asset_ReadinessProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Reinforcement Requests | Reinforcement_RequestsProjection | RequestReinforcement | Event-driven alarms; configured normal/fast telemetry |
| Supply Network | Supply_NetworkProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Territory and Conquest Map | Territory_and_Conquest_MapProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Base and Installation Status | Base_and_Installation_StatusProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Campaign Objectives | Campaign_ObjectivesProjection | SubmitCampaignEvidence | Event-driven alarms; configured normal/fast telemetry |
| Faction Intelligence | Faction_IntelligenceProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Loss and Replacement | Loss_and_ReplacementProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Strategic Event Timeline | Strategic_Event_TimelineProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
...
- Fleet cannot bypass local authority
- Scenarium remains sole campaign authority
- Shared data retains provenance
- Orders are idempotent/versioned
- Synchronised operations have commit/abort
