# Weapons — Developer Implementation Guide
...
Weapons owns readiness, arming, firing solutions, ready ammunition, salvo execution, and weapon safety. Tactical owns target intent, Command owns ROE, and Logistics owns production/storage.
...
- IWeaponRegistry
- IFireControlService
- IAmmunitionReadinessService
- IMissileService
- IPointDefenceService
- ISalvoService
- IWeaponSafetyService
...
| --- | --- | --- | --- |
| Weapons Dashboard | Weapons_DashboardProjection | ArmWeaponGroup, SafeWeaponGroup, AssignWeaponTarget, SafeAllWeapons | Event-driven alarms; configured normal/fast telemetry |
| Fire Control | Fire_ControlProjection | RequestFire, FireAuthorisedShot, CeaseFire | Event-driven alarms; configured normal/fast telemetry |
| Weapon Groups | Weapon_GroupsProjection | ArmWeaponGroup, SafeWeaponGroup, AssignWeaponTarget, SafeAllWeapons | Event-driven alarms; configured normal/fast telemetry |
| Turrets | TurretsProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Fixed Weapons | Fixed_WeaponsProjection | ArmWeaponGroup, SafeWeaponGroup, AssignWeaponTarget, SafeAllWeapons | Event-driven alarms; configured normal/fast telemetry |
| Missiles | MissilesProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Railguns and Artillery | Railguns_and_ArtilleryProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Point Defence | Point_DefenceProjection | SetPointDefenceDoctrine | Event-driven alarms; configured normal/fast telemetry |
| Ammunition | AmmunitionProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Firing Arcs | Firing_ArcsProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Target Profiles | Target_ProfilesProjection | AssignWeaponTarget | Event-driven alarms; configured normal/fast telemetry |
| Weapon Safety | Weapon_SafetyProjection | ArmWeaponGroup, SafeWeaponGroup, AssignWeaponTarget, SafeAllWeapons | Event-driven alarms; configured normal/fast telemetry |
| Salvo Programming | Salvo_ProgrammingProjection | ExecuteSalvo | Event-driven alarms; configured normal/fast telemetry |
| Weapons Diagnostics | Weapons_DiagnosticsProjection | ArmWeaponGroup, SafeWeaponGroup, AssignWeaponTarget, SafeAllWeapons | Event-driven alarms; configured normal/fast telemetry |
| Weapons Maintenance | Weapons_MaintenanceProjection | ArmWeaponGroup, SafeWeaponGroup, AssignWeaponTarget, SafeAllWeapons | Event-driven alarms; configured normal/fast telemetry |
...
- No weapon fires from client-only decision
- Final safety checks occur server-side
- Ready ammo differs from cargo total
- Safe-all works without Tactical
- Every shot is fully audited
