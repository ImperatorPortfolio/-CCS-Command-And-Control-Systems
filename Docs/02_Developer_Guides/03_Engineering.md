# Engineering — Developer Implementation Guide
...
Engineering owns technical availability, safety limits, power distribution, fuel and conveyor topology, equipment isolation, maintenance, and technical procedures. Helm owns manoeuvre intent and Weapons owns firing intent.
...
- IPowerPlantService
- IPowerDistributionService
- IFuelSystemService
- IPropulsionAvailabilityService
- IConveyorTopologyService
- IMechanicalSystemService
- IJumpDriveService
- IThermalService
- IShieldService
- IMaintenanceService
...
| --- | --- | --- | --- |
| Engineering Dashboard | Engineering_DashboardProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Power Grid | Power_GridProjection | StartPowerSource, StopPowerSource, TransferPowerBus | Event-driven alarms; configured normal/fast telemetry |
| Reactors | ReactorsProjection | ScramReactorGroup | Event-driven alarms; configured normal/fast telemetry |
| Batteries | BatteriesProjection | SetBatteryPolicy | Event-driven alarms; configured normal/fast telemetry |
| Renewable Generation | Renewable_GenerationProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Hydrogen Engines | Hydrogen_EnginesProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Power Distribution | Power_DistributionProjection | StartPowerSource, StopPowerSource, TransferPowerBus | Event-driven alarms; configured normal/fast telemetry |
| Load Shedding | Load_SheddingProjection | ApplyLoadShedStage | Event-driven alarms; configured normal/fast telemetry |
| Emergency Power | Emergency_PowerProjection | StartPowerSource, StopPowerSource, TransferPowerBus | Event-driven alarms; configured normal/fast telemetry |
| Propulsion Overview | Propulsion_OverviewProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Thruster Groups | Thruster_GroupsProjection | ScramReactorGroup | Event-driven alarms; configured normal/fast telemetry |
| Fuel Feed | Fuel_FeedProjection | SetFuelTankPolicy | Event-driven alarms; configured normal/fast telemetry |
| Attitude Control | Attitude_ControlProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Hydrogen Systems | Hydrogen_SystemsProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Oxygen Systems | Oxygen_SystemsProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Conveyor Network | Conveyor_NetworkProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Mechanical Systems | Mechanical_SystemsProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Gravity Systems | Gravity_SystemsProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Jump Drives | Jump_DrivesProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Thermal Management | Thermal_ManagementProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Shield Systems | Shield_SystemsProjection | Read-only projection or domain-service operation; no direct page-to-adapter command is permitted. | Event-driven alarms; configured normal/fast telemetry |
| Structural Loads | Structural_LoadsProjection | ApplyLoadShedStage | Event-driven alarms; configured normal/fast telemetry |
| Maintenance and Diagnostics | Maintenance_and_DiagnosticsProjection | RunDiagnostic | Event-driven alarms; configured normal/fast telemetry |
...
- Cold-start and shutdown can be completed without vanilla terminal
- Commanded and actual state are distinct
- Black-start recovers critical loads safely
- Helm receives truthful control authority
- Conveyor diagnostics identify blocking segment
